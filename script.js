...
function setupEventListeners() {
    // Filter events
    elements.woodTypeFilter?.addEventListener('change', applyFilters);
    elements.certificateFilter?.addEventListener('change', applyFilters);
    elements.gradeFilter?.addEventListener('change', applyFilters);
    elements.thicknessFilter?.addEventListener('change', applyFilters);
...
function setupCartEvents() {
    elements.openCartBtn?.addEventListener('click', showCartModal);
...
function populateFilters() {
    ...
    populateSelect(elements.gradeFilter, grades);
    populateSelect(elements.thicknessFilter, thicknesses);
}
...
function renderProducts() {
    ...
    const productsHTML = filteredData.map(product => createProductCard(product)).join('');
...
function showProductModal(product) {
    ...
    elements.modalTitle.textContent = `${product.Logs} ${product.Size}`;
    elements.modalBody.innerHTML = createModalContent(product);
